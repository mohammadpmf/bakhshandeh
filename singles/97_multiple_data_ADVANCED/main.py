...
def get_info():
    ...
    this_pc = ComputerInfo(coll, device_id, cpu_usage, ram_usage, ram_used, ram_free, ram_used2, ram_free2)
    status = 200
    if len(errors)!=0:
        context = { 'errors': errors }
        return render_template("home.html", **context)
    if cpu_usage != 0:
        return f"Status Code is: {status}{'<br>'*2}{this_pc.set_and_get_cpu_percent()}", status
    elif ram_usage != 0:
        return f"Status Code is: {status}{'<br>'*2}{this_pc.set_and_get_ram_percent()}", status
    elif ram_used != 0 and ram_free != 0:
        return f"Status Code is: {status}{'<br>'*2}{this_pc.set_and_get_ram_usage()}", status
    elif ram_used2 != 0 and ram_free2 != 0:
        return f"Status Code is: {status}{'<br>'*2}{this_pc.set_and_get_ram_usage2()}", status
    status = 404
